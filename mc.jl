...
function add_exch(sim::MonteCarlo; J=1, J1=0, J_interlayer=0)
    T = _cuda_using_double.x ? Float64 : Float32
    mesh = sim.mesh
    J_array = Array([T(1), T(1), T(1), T(1), T(1), T(1), T(1), T(1)])
#    J1_array = Array([T(1), T(1), T(1), T(1), T(1), T(1), T(0), T(0)])
    for i in 1:6
        J_array[i] = T(J_array[i] .* J/k_B)
#        J1_array[i] = T(J1_array[i] .* J1/k_B)
    end
    J_array[7] = J_interlayer/k_B
    J_array[8] = J_interlayer/k_B
    copyto!(sim.exch.J, J_array)
#    copyto!(sim.exch.J1, J1_array)
    return nothing
end
...
function matrix_time(B::Any)
    A = zeros(3,3)
    A[1,1],A[1,2],A[2,1],A[2,2],A[3,3] = (1/2,-sqrt(3)/2,sqrt(3)/2,1/2,1)
    a,b,c = ((A[1,1],A[1,2],A[1,3]),(A[2,1],A[2,2],A[2,3]),(A[3,1],A[3,2],A[3,3]))
    d,e,f = (sum(a .* B),sum(b .* B),sum(c .* B))
    return [d,e,f]
end
function sixrotate(B::Any,i::T) where {T <: Number}
    c = collect(B)
    if i == 0
        return Tuple(c)
    elseif i <= 5
...
function add_dmi(sim::MonteCarlo, D_ratio::AbstractFloat, D_right::Tuple{Float64, Float64, Float64}, type::String)
    dmi = sim.dmi
    T = _cuda_using_double.x ? Float64 : Float32
    D = zeros(3,8)
#    D1 = zeros(3,8)
    for i in 1:8
        D[1,i],D[2,i],D[3,i] = sixrotate(D_right,i-1)
#        D1[1,i],D1[2,i],D1[3,i] = sixrotate(D1_topright,i-1)
    end
    D = D ./ k_B
#    D1 = D1 ./ k_B
    sim.D_ratio = D_ratio
