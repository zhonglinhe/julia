...
function add_dE_exch_dmi_triangular_mesh_kernel!(m::CuDeviceArray{T, 1}, next_m::CuDeviceArray{T, 1},
                              ...
                              nx::Int64,ny::Int64,nz::Int64,bias::Int64,D_ratio::AbstractFloat) where {T<:AbstractFloat}
    ...
    if index <= nx*ny*nz && shape[index]
        ...
        for j = 1:n_ngbs
            ...
            @inbounds J = J0[j]
            @inbounds Dx = d_ratio*D0[1, j]
            @inbounds Dy = d_ratio*D0[2, j]
            @inbounds Dz = d_ratio*D0[3, j]
            ...
            if id>0
                ...
                @inbounds sy = m[k+1]
                @inbounds sz = m[k+2]
                delta_E -= J*(dmx*sx + dmy*sy + dmz*sz) + (0)*dmz*sz #exchange,lamda
                delta_E += volume(Dx, Dy, Dz, dmx, dmy, dmz, sx, sy, sz) #DMI
            end
        end
        @inbounds dE[index] += delta_E
    end
...
function add_total_E_exch_dmi_triangular_mesh_kernel!(m::CuDeviceArray{T, 1},
                              ...
                              n_ngbs::Int64, J0::CuDeviceArray{T, 1},
                              D0::CuDeviceArray{T, 2},
                              nxyz::Int64,nx::Int64, ny::Int64,nz::Int64,D_ratio::AbstractFloat) where {T<:AbstractFloat}
    ...
    if index <= nxyz
        ...
        for j = 1:n_ngbs
            ...
            if id>0
                ...
                @inbounds sy = m[k+1]
                @inbounds sz = m[k+2]
                delta_E -= J*(mx*sx + my*sy + mz*sz) + (0)*mz*sz #exchange,lamda
                delta_E += volume(Dx, Dy, Dz, mx, my, mz, sx, sy, sz) #DMI
            end
        end
        @inbounds dE[index] += 0.5*delta_E
    end
